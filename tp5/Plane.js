...
function Plane(scene, nrDivs, ratioX, ratioY) {
	...
	var tmp = gcd(this.ratioX, this.ratioY);
	this.ratioX = this.ratioX/tmp;
	this.ratioY = this.ratioY/tmp;
...
Plane.prototype.initBuffers = function() {
	...
	*/
	...
	var dif = Math.abs(this.ratioX - this.ratioY);
	var yCoord = 0.5;
	...
	for (var j = 0; j <= this.nrDivs; j++) 
	{
		...
		for (var i = 0; i <= this.nrDivs; i++) 
		{
			...
			if(this.ratioX < this.ratioY)
				this.texCoords.push(i/this.nrDivs, j*(1+dif)/this.nrDivs - dif/2);
			else{
				this.texCoords.push(i*(1 + dif)/this.nrDivs - dif/2, j/this.nrDivs);
			}
